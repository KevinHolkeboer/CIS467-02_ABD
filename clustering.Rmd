...
#Function to check the bevType and update the columns (NEW BEVERAGE)
bevTypeCheck = function(input_bevType){
  #case_when() to update the new individual bevType columns
  case_when(
    input_bevType == "NEW AGE BEVERAGE" ~ 1,
    input_bevType == "CRAFT BEER" ~ 1,
    input_bevType == "IMPORT BEER" ~ 1,
    input_bevType == "PREMIUM BEER" ~ 1,
    input_bevType != "NEW AGE BEVERAGE" ~ 0
  )
}

#Tests the function
bevTypeCheck("CRAFT BEER")

db2 <- add_row(db, `NEW AGE BEVERAGE` = bevTypeCheck("NEW AGE BEVERAGE"))
...
#Testing the function and it works
dba <- frequencyConvert_Existing_Data(dba)
